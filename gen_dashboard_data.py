...
def clean_reason_text(reason: str) -> str:
    ...
    # 第 3 步：清理被删后残留的孤立符号（- : , . 空格），让 reason 干净
    s = re.sub(r"^[\s\-_:：,，。.]+", "", s)
    s = re.sub(r"[\s\-_:：,，。.]+$", "", s)
...
# v8 改进：占位符全改成空字符串，激进删除子描述变体
FAMILY_SUB_NORMALIZE_RULES = {
    # === 预定失败 - 询价异常族 ===
    "预定失败-询价异常-反采同程": [
        # 业务子描述变体（不足 / 没有匹配航班 / 账号为空 / 价格详情查询失败）直接删
    ...
    "预定失败-询价失败-航班管家": [
        (r"(不足|没有匹配到对应航班|没有匹配到航班|账号为空|价格详情查询失败|非法的.*token)", ""),
    ],
    ...
    "预定失败-预定异常": [
        ...
        (r"(航班筛选异常|Cannot invoke.*iterator|Connection.*timed out|Connection.*refused|Read timed out|Unexpected.*end of file|IOException.*)", ""),
        (r"(当前航班已存在姓名、证件号相同的旅客|当前会员已存在两个尚未支付的订单|当前设备操作存在异常)", ""),
        (r"(下单成功.*查询订单详情失败|下单成功.*获取TradeId失败|下单成功.*金额获取失败)", ""),
        (r"(已生单[：:].*【已生单失败】.*?(?=反采|春秋|港版|MF|agoda))", ""),
    ],

    # === 取票失败族 ===
    "取票失败-取票异常": [
        # 各种"取票失败 当前状态为:..." 的子状态归一（保留前缀，删除子状态）
        (r"(当前订单状态为[：:][^,，：:]+)", "当前订单状态为:"),
        (r"(当前状态为[：:][^,，：:]+)", "当前状态为:"),
        (r"(用户令牌错误或用户登录已过期)", ""),
    ],
    ...
    "验真失败-验真异常": [
        # 各种报错信息归一（直接删）
        (r"(抱歉 未找到机票信息|Connection refused|Connection timed out|Read timed out|ConnectException[^,，]*|系统维护中[^,，]*|9C行李额查询失败|401|相同证件号每日查询次数已达上限[^,，]*|尊敬的用户[^,，]*)", ""),
    ],

    # === 支付失败族 ===
    "支付失败-支付异常": [
        (r"(CHECK_LOGIN_CITY_ERROR|登录失败|未知错误|获取支付金额失败|.*超时|SSL handshake.*failed|certificate verify.*failed)", ""),
    ],

    # === 平台失败 - 状态监测族 ===
    "平台失败-状态监测": [
        (r"状态校验失败-(待出票|验证中|未出票|出票失败|已退款|出票成功|出票中|.*)", "状态校验失败-"),
    ],
}
